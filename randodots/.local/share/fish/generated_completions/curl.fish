...
complete -c curl -l connect-to -d 'For a request to the given "HOST1:PORT1" pair, connect to "HOST2:PORT2" inste…'
...
complete -c curl -l disallow-username-in-url -d 'This tells curl to exit if passed a URL containing a username'
...
complete -c curl -l http3 -d '(HTTP) Tells curl to try HTTP/3 to the host in the URL, but fallback to earli…'
...
complete -c curl -l ssl-allow-beast -d '(TLS) This option tells curl to not work around a security flaw in the SSL3 a…'
complete -c curl -l ssl-auto-client-cert -d '(TLS) (Schannel) Tell libcurl to automatically locate and use a client certif…'
complete -c curl -l ssl-no-revoke -d '(TLS) (Schannel) This option tells curl to disable certificate revocation che…'
complete -c curl -l ssl-reqd -d '(FTP IMAP POP3 SMTP LDAP) Require SSL/TLS for the connection'
complete -c curl -l ssl-revoke-best-effort -d '(TLS) (Schannel) This option tells curl to ignore certificate revocation chec…'
...
complete -c curl -l expand-data -d https://example
